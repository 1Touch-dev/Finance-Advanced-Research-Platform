...
**Last push:** `68c94dd` — full 15-document review in `15th_June.md`
...
| **Phase 3 — Layer 1 build** | 🟢 **ACTIVE — starts today** |
...
| **17 federal connectors** | ✅ Live — primary data layer for Layer 1 |
| **Report engine (existing)** | ⚠️ Partial — models/APIs exist; no James-style dossier pipeline yet |
...
## 7) What exists in codebase vs gaps

### Already built (reuse for Layer 1)

| Component | Location | Notes |
|-----------|----------|-------|
| Report models | `apps/api/app/models/reports.py` | Report, ReportSection, claims, bundles |
| Report APIs | `apps/api/app/api/reports.py` | CRUD, claim verification stubs |
| Graph APIs | `apps/api/app/api/graph.py` | expand, path, related — partial |
| Entity resolution | `apps/api/app/api/entities.py` | fuzzy match, merge proposals |
| 17 federal connectors | `packages/connectors/us/` | Live; ETL depth varies |
| OpenAI client | `apps/api/app/services/openai_client.py` | Ready for narrative |
| Registry API | `/registry/*` | 202 records, 51 jurisdictions |

### Gaps to close for Layer 1 demo

| Gap | Priority | Notes |
|-----|----------|-------|
| **End-to-end report orchestrator** | P0 | No job that runs connectors → graph → report JSON |
| **Federal ETL → relationships** | P0 | Connectors ingest records; weak link to `relationships` table |
| **Entity resolution across sources** | P0 | Same person across SEC/FEC/registry |
| **Report template engine** | P0 | Structured JSON → James-style sections |
| **PDF export** | P1 | reportlab stub exists; needs template |
| **UI "Generate report" button** | P1 | Frontend trigger + progress |
| **Ownership tree crawler** | P2 | OpenOwnership / OpenCorporates — not blocking Palantir demo |
| **FinCEN BOI** | P2 | Future enhancement |
...
| # | Task | Owner | Status |
|---|------|-------|--------|
| 1 | Create **`16th_June.md`** (this doc) — James sign-off + demo theme + task list | Abhishek | ✅ |
| 2 | Draft **Layer 1 report section schema** (JSON structure for 7 sections + claim/evidence model) | — | 🔲 |
| 3 | Finalize **Thiel demo seed list** (~10 nodes; Palantir as first spike) | — | 🔲 |
| 4 | **Spike Palantir end-to-end** — SEC CIK → FEC → USASpending → FARA → what returns vs gaps | — | 🔲 |
| 5 | Design **Layer 1 API/workflow** — e.g. `POST /reports/intelligence/generate` extending existing `/reports` | — | 🔲 |
| 6 | Map **connector output → relationship edges** for demo entity | — | 🔲 |
...
### Blockers
- **None** for Layer 1 start
- OIDC credentials — nice-to-have, not blocking demo

---

## 9) Pending tasks — revised priority (post James sign-off)

### Sprint 1 — Layer 1 foundation (this week)

| # | Task | Effort |
|---|------|--------|
| L1.1 | Report section JSON schema + confidence tags | Medium |
| L1.2 | Intelligence report orchestrator (entity in → connectors → graph) | High |
| L1.3 | Wire USASpending, FEC, FARA, SEC, OFAC → `relationships` for demo entity | High |
| L1.4 | Entity resolution spike — officer/person across entities | Medium |
| L1.5 | GPT-4o narrative writer (claims-only, no hallucination) | Low |
| L1.6 | First Palantir dossier — cited JSON + PDF | Medium |

### Sprint 2 — Demo polish (next week)

| # | Task | Effort |
|---|------|--------|
| L1.7 | Expand graph to full Thiel seed list (10 nodes) | Medium |
| L1.8 | Report UI — expandable sections + source links | Medium |
| L1.9 | PDF template matching James doc style | Medium |
| L1.10 | James review + feedback loop | — |
...
| File | Role |
|------|------|
| `16th_June.md` | This handoff — Layer 1 kickoff |
...
| `apps/api/app/api/reports.py` | Report APIs (extend for Layer 1) |
| `apps/api/app/api/graph.py` | Graph APIs |
| `packages/connectors/us/` | Federal + state connectors |

---

## 12) Suggested message to James (optional update)

> Hi James,
>
> Confirmed — we're starting **Layer 1 Entity Network Reports** now, without waiting on CA or Cobalt.
>
> **First demo:** Peter Thiel / tech / AI / defense network — starting with **Palantir** as the anchor (richest public data: SEC, contracts, FEC, lobbying), then expanding to Anduril, Founders Fund portfolio, HawkEye 360, Erebor, and related nodes from your docs.
>
> **Output:** cited dossier with ownership, investors, government contracts, officer network, risk flags, and a short AI summary — same structure as your ARG and Asia Defense examples.
>
> We'll share the first draft dossier for your review once the Palantir spike is done. No blockers on our side.

---

*End of 16 June handoff — Layer 1 active, demo theme locked*
