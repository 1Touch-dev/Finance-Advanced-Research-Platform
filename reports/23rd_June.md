...
|---|------|---------------|--------|
| P1-1 | Entity profile: **Financial tab** — 20-yr revenue chart (Recharts), Beneish M-Score gauge, Altman Z-Score, Insider transactions | `pages/entities/[id].js`, `finance_connector.py` | ✅ Done |
| P1-2 | Entity profile: **People & Contacts tab** — Apollo executives, emails, titles, LinkedIn URLs | `pages/entities/[id].js`, `apollo_connector.py` | ✅ Done |
| P1-3 | Entity profile: **Social Footprint tab** — platform cards (Twitter/LinkedIn/Instagram/YouTube), sentiment chart, key quotes | `pages/entities/[id].js`, `apify_connector.py` | ✅ Done |
| P1-4 | Entity profile: **RAG Chat tab** — move floating chat into dedicated entity tab with suggested questions | `pages/entities/[id].js`, `chat.py` | ✅ Done |
| P1-5 | Intelligence report: **Timeline View** toggle — all events on Vis.js timeline | `pages/intelligence.js` | ✅ Done |
| P1-6 | Intelligence report: **Graph View** toggle — Cytoscape embedded inside report view | `pages/intelligence.js` | ✅ Done |
| P1-7 | **Word (.docx) export** — `python-docx` backend route + button in UI | `api/intelligence.py`, `pdf_service.py` | ✅ Done |
| P1-8 | **Excel (.xlsx) export** — `openpyxl` backend route + button in UI | `api/intelligence.py` | ✅ Done |
| P1-9 | **PowerPoint (.pptx) export** — `python-pptx` backend route + button in UI | `api/intelligence.py` | ✅ Done |
| P1-10 | **`/intelligence/{id}` dedicated report page** — each saved report gets its own URL | `pages/intelligence/[id].js` | ✅ Done |
| P1-11 | **`/entities/{id}/chat` dedicated page** — full-page RAG chat per entity | `pages/entities/[id]/chat.js` | ✅ Done |
| P1-12 | `POST /entities/compare-chat` route | `api/chat.py` | 🔲 Pending |
| P1-13 | "Generate PDF from chat" button | `pages/intelligence.js` | 🔲 Pending |
| P1-14 | **`/tracking/alerts` page + API** — alert inbox with severity, acknowledge, snooze | `pages/tracking/alerts.js`, `api/tracking.py` | ✅ Done |
| P1-15 | **API Status Panel component** — per-source green/yellow/red on every report + entity page | `components/ApiStatusPanel.js` | ✅ Done |
| P1-16 | **Date-range filter** on report claims (Last 30d / 1yr / All time) | `pages/intelligence.js` | ✅ Done |
| P1-17 | **Search-within-report highlight** — matching claims highlighted yellow on search | `pages/intelligence.js` | ✅ Done |

### P2 — New data connectors (all free APIs)

| # | Connector | Key to register | Status |
|---|-----------|----------------|--------|
| P2-1 | **FINNHUB** — real-time quotes, financial statements | `finnhub.io` (free) | ✅ Done |
| P2-2 | **FMP (Financial Modeling Prep)** — income statement, balance sheet, cash flow | `financialmodelingprep.com` (free, 250 req/day) | ✅ Done |
| P2-3 | **Alpha Vantage** — OHLCV + technicals | `alphavantage.co` (free, 25 req/day) | ✅ Done |
| P2-4 | **FRED** — Federal Reserve macro data (inflation, rates, GDP) | `fred.stlouisfed.org` (free, unlimited) | ✅ Done |
| P2-5 | **NewsAPI** — breaking news by entity name | `newsapi.org` (free, 100 req/day) | ✅ Done |
| P2-6 | **The Guardian API** — global news search | `open-platform.theguardian.com` (free, unlimited) | ✅ Done |
| P2-7 | **NYT Article Search** — historical news archive | `developer.nytimes.com` (free, 500 req/day) | ✅ Done |
| P2-8 | **GDELT** — global news events + sentiment | No key needed | ✅ Done |
| P2-9 | **UK Companies House** — UK officers, filings, PSC data | `developer.company-information.service.gov.uk` (free, 600 req/min) | ✅ Done |
| P2-10 | **ICIJ Offshore Leaks** — Panama/Paradise/Pandora Papers | No key needed | ✅ Done |
| P2-11 | **ALEPH/OCCRP** — leaked document datasets | `aleph.occrp.org` (free non-commercial) | ✅ Done |
| P2-12 | **OpenSanctions** — confirm live (key already in `.env`) | Already have: `f13ceaf124...` | ✅ Done |

### P3 — UX completeness

| # | Task | Target file(s) | Status |
|---|------|---------------|--------|
| P3-1 | **`/saved` reports library page** — list all saved reports, search, filter, delete | `pages/saved.js` | ✅ Done |
| P3-2 | **Contract treemap by agency** on entity profile (Recharts Treemap, clickable) | `pages/entities/[id].js` | ✅ Done |
| P3-3 | **Lobbying spend bar chart** on entity profile (by year + by issue) | `pages/entities/[id].js` | ✅ Done |
| P3-4 | **"Add to Tracking" button** on every entity card and every report page | `pages/intelligence.js`, `pages/entities/[id].js` | ✅ Done |
| P3-5 | **Comparison page: Grouped Bar Chart** (financial metrics by year) | `pages/compare.js` | ✅ Done |
| P3-6 | **Comparison page: Timeline Overlay** (key events for all entities on same axis) | `pages/compare.js` | ✅ Done |
| P3-7 | **PM2 cron** for daily digest at 6 AM UTC | `ecosystem.config.js` | ✅ Done |
