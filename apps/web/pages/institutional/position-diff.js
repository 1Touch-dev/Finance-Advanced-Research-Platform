...
import {
  ...
  POSITION_DIFF_SORT_OPTIONS,
  POSITION_DIFF_STATUS_LABELS,
  POSITION_DIFF_STATUS_OPTIONS,
...
const INITIAL_FORM = {
  ...
  limit: String(POSITION_DIFF_DEFAULTS.limit),
  offset: String(POSITION_DIFF_DEFAULTS.offset),
}
    ...
    <div className={sectionStyles.warningList}>
      {warnings.map((warning, index) => (
        ...
        >
          ...
          <p className={sectionStyles.warningMessage}>{warning.message || EMPTY}</p>
        </div>
      ))}
    </div>
  )
      ...
      {!positions || positions.length === 0 ? (
        ...
        <div className={sectionStyles.highlightGrid}>
          {positions.map((position, index) => (
            <article
              key={`${title}-${position.issuer_name || 'issuer'}-${position.cusip || index}`}
              className={sectionStyles.highlightCard}
            >
              <div className={sectionStyles.highlightTop}>
                <h3>{position.issuer_name || EMPTY}</h3>
                <span className={sectionStyles.statusBadge}>
                  {POSITION_DIFF_STATUS_LABELS[position.status] || position.status || EMPTY}
                </span>
              </div>
              ...
              <div className={sectionStyles.highlightNumbers}>
                <span>Shares: {formatNumber(position.share_diff, { maximumFractionDigits: 0 })}</span>
...
function SupplementalAmendments({ title, amendments }) {
  const items = Array.isArray(amendments) ? amendments : []

  return (
    <div className={sectionStyles.amendmentBlock}>
      <h4>{title}</h4>
      {items.length === 0 ? (
        <p className={styles.empty}>No supplemental amendments</p>
      ) : (
        <div className={sectionStyles.amendmentList}>
          {items.map((amendment, index) => (
            <div
              key={`${amendment.accession_number || title}-${index}`}
              className={sectionStyles.amendmentCard}
            >
              <DetailList
                items={[
                  { label: 'Accession Number', value: amendment.accession_number || EMPTY },
                  { label: 'Filing Date', value: formatDate(amendment.filing_date) },
                  { label: 'Form', value: amendment.form || EMPTY },
                  { label: 'Amendment Type', value: amendment.amendment_type || EMPTY },
                ]}
              />
            </div>
          ))}
  ...
  return (
    <div className={sectionStyles.filingGrid}>
      <div className={sectionStyles.filingCard}>
        ...
        <DetailList
          items={[
            { label: 'Accession Number', value: current?.accession_number || EMPTY },
            { label: 'Filing Date', value: formatDate(current?.filing_date) },
            { label: 'Form', value: current?.form || EMPTY },
            { label: 'Amendment Type', value: current?.amendment_type || EMPTY },
          ]}
        />
        <SupplementalAmendments
          title="Current Supplemental Amendments"
          amendments={filings?.current_supplemental_amendments}
        />
      </div>
      <div className={sectionStyles.filingCard}>
        ...
        <DetailList
          items={[
            { label: 'Accession Number', value: previous?.accession_number || EMPTY },
            { label: 'Filing Date', value: formatDate(previous?.filing_date) },
            { label: 'Form', value: previous?.form || EMPTY },
            { label: 'Amendment Type', value: previous?.amendment_type || EMPTY },
          ]}
        />
        <SupplementalAmendments
          title="Previous Supplemental Amendments"
    ...
    <div className={sectionStyles.tableWrap}>
      <table className={sectionStyles.table}>
        ...
        <tbody>
          {positions.map((position, index) => (
            <tr
              key={`${position.issuer_name || 'issuer'}-${position.cusip || position.current_accession_number || position.previous_accession_number || index}`}
            >
              <td className={sectionStyles.issuerCell}>
                <span>{position.issuer_name || EMPTY}</span>
                {position.security_title ? (
                  <span className={sectionStyles.cellSubtle}>{position.security_title}</span>
                ) : null}
              </td>
              <td>{position.ticker || EMPTY}</td>
              <td>{position.cusip || EMPTY}</td>
        ...
        <section className={styles.panel}>
          ...
          <form className={styles.controls} onSubmit={handleSubmit}>
            <div className={sectionStyles.formGrid}>
              ...
              <label className={styles.label}>
                Status
                <select className={styles.select} value={form.status} onChange={updateField('status')} disabled={loading}>
                  {POSITION_DIFF_STATUS_OPTIONS.map((option) => (
              ...
              <label className={styles.label}>
                Sort Field
                <select className={styles.select} value={form.sort_by} onChange={updateField('sort_by')} disabled={loading}>
                  {POSITION_DIFF_SORT_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
              </label>
              <label className={styles.label}>
                Sort Direction
                <select className={styles.select} value={form.sort_dir} onChange={updateField('sort_dir')} disabled={loading}>
                  {POSITION_DIFF_SORT_DIRECTION_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>{option.label}</option>
                  ))}
                </select>
              </label>
            ...
            <p className={styles.subtle}>
              Required: institution CIK. Optional: periods, status, ticker, CUSIP, sorting, and pagination.
            </p>
          </form>
        </section>
        ...
        {response ? (
          <div className={sectionStyles.sectionStack}>
            ...
            <section className={styles.panel}>
              ...
              <DetailList
                items={[
                  { label: 'Name', value: response.institution?.name || EMPTY },
                  { label: 'CIK', value: response.institution?.cik || EMPTY },
                ]}
              />
            </section>
