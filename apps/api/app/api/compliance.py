...
def create_policy(name: str, rules: Optional[dict] = None, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    p = Policy(name=name, rules=rules or {})
    db.add(p); db.commit(); db.refresh(p)
    return {"id": p.id}

@router.post('/export/request')
def request_export(report_id: int, requested_by: Optional[str] = None, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    e = ExportApproval(report_id=report_id, requested_by=requested_by)
    db.add(e); db.commit(); db.refresh(e)
    return {"id": e.id, "status": e.status}

@router.post('/export/{eid}/approve')
def approve_export(eid: int, approve: bool = True, notes: Optional[str] = None, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
